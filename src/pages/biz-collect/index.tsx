...
import { StatusColumn } from '@/components/SmartTableColumnStyle'
import {formatMoney_util, handleAxiosError, RoundValue} from '@/util'
import 'react-loading-skeleton/dist/skeleton.css'
import {useDashboardData} from "@/hooks";
import {spans} from "next/dist/build/webpack/plugins/profiling-plugin";
import classNames from "classnames";
...
const BizCollect: NextPageWithLayout = (props) => {
  ...
  const [selectedStartDate, setSelectedStartDate] = useState<Date | null>(null);
  const [selectedEndDate, setSelectedEndDate] = useState<Date | null>(null);
  const [bizCollectDBData, setBizCollectData] = useState<BizCollectDashboardData>({})
  ...
  const fetchDashboardData = async (filters: Record<string, any>) => {
    setIsLoading(true)
    setIsPending(true)
    try {
      ...
      setBizCollectData(response.data)
  ...
  useEffect(() => {
    ...
    console.log('✅ bizCollectData', bizCollectDBData)
  }, [filters])
